...
const ANDROID_NOTIFICATION_STYLE = {
  ...
  largeIcon: 'ic_treino_large',
  iconColor: '#F28C28',
  group: 'treino-reminders',
  summaryText: 'Treino 3 Meses',
  largeBody: 'Seu treino guiado esta pronto. Intervalos, coach e foco total.',
  inboxList: [
    'Treino do dia pronto',
    'Coach com voz e intervalos',
...
function speak(text, force = false) {
  ...
  if (isNative()) {
    ...
    if (TTS && TTS.speak) {
      TTS.speak({ text, lang: 'pt-BR', rate: 1.0 }).catch(() => {});
      return true;
    }
    return false;
  }
...
function testVoice() {
  if (isNative() && !getTextToSpeech()) {
    setSettingsStatus('TextToSpeech nao instalado no Android. Rode cap:sync e gere o APK novamente.');
    return;
  }
  if (isNative() && getTextToSpeech()) {
...
function buildAndroidNotification(base) {
  return { ...ANDROID_NOTIFICATION_STYLE, ...base };
}
...
async function scheduleReminders() {
  ...
  if (isNative()) {
    try {
      ...
      const notifications = settings.reminders.days.map((day, index) => buildAndroidNotification({
        id: 1000 + index,
        title: 'Bora treinar agora',
        body: 'Seu treino guiado esta pronto. Vamos nessa.',
        channelId: 'treino',
        sound: 'default',
        schedule: {
...
async function showWebNotification() {
  ...
  if ('serviceWorker' in navigator) {
    ...
    if (reg && reg.active && reg.showNotification) {
      await reg.showNotification('Bora treinar agora', { body: 'Seu treino guiado esta pronto. Vamos nessa.' });
      return true;
    }
  }
  return false;
  ...
  if (isNative()) {
    try {
      ...
      await LocalNotifications.schedule({
        notifications: [buildAndroidNotification({
          id: 999,
          title: 'Bora treinar agora',
          body: 'Seu treino guiado esta pronto. Vamos nessa.',
          channelId: 'treino',
...
async function ensureAndroidNotificationChannel(LocalNotifications) {
  if (!LocalNotifications || !LocalNotifications.createChannel) return;
  try {
    const existing = LocalNotifications.listChannels ? await LocalNotifications.listChannels() : null;
    const hasChannel = existing && existing.channels && existing.channels.some((c) => c.id === 'treino');
    if (hasChannel) return;
    await LocalNotifications.createChannel({
      id: 'treino',
      name: 'Treino',
      description: 'Lembretes do treino',
      importance: 5,
      sound: 'default',
      vibration: true,
      lights: true,
